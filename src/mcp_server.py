...
def diagnose(corpus_dir, threads_path=None, embed=False, time_context=None,
    ...
    if ratio > _MTIME_WARN_RATIO or outranks:
        ...
        add(WARN, "时间戳来源",
            ...
            # ⚠ **这句指的是「上一站」，不是这个目录**（2026.08.12 补，一例外部实测）：
            # 比例这么高时，十有八九不是用户一份份文件漏写了日期，而是**上游那个把导出
            # 转成 md 的脚本整批吞掉了时间字段**——那一例的形状是：导出源 58 个窗口
            # 全都有日期，但 48 个是 13 位毫秒时间戳，而转换脚本只解 ISO 字符串，
            # 剩下的掉进一个空的 except 变成 unknown，**九成七的块因此退到 mtime，
            # 全程零报错**。体检报出比例之后，用户第一反应是回去一份份补日期，
            # 那是在最贵的那一头修一个在最便宜那一头能一次修完的东西。
            # 我们自己的 memory_import 已经两种都认（`_any_ts`），
            # 但**用户自己写的转换脚本不在我们手里**，所以这句提醒得留在这儿。
            + ("。⚠ 这份语料如果是脚本从聊天导出转出来的，**先回去查那个转换脚本，"
               "别急着一份份补日期**：导出里的时间常常是 13 位毫秒时间戳，"
               "而只认 ISO 字符串的解析会把它们整批吞掉且不报错"
               "（一例外部实测：58 个窗口里 48 个这样，九成七的块因此落到这一档）"
               if ratio > _MTIME_WARN_RATIO else ""))
    else:
        add(OK, "时间戳来源", detail)
    if index.date_order:
...
def _selftest():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        #    【指向上一站·变异靶心】（2026.08.12 补，一例外部实测逼出来的）：
        #    比例这么高时，病根几乎一定在**上游那个转换脚本**，不在这个目录里。
        #    那一例的形状是导出源 58 个窗口全都有日期、48 个是 13 位毫秒戳，
        #    只解 ISO 的脚本把它们整批吞成 unknown，九成七的块退到 mtime、零报错；
        #    而用户看完体检的第一反应是回去一份份补日期——**在最贵的那头修一个
        #    在最便宜那头能一次修完的东西**。
        #    变异：把 detail 里那句「先回去查那个转换脚本」删掉 → 这条红。
        assert "转换脚本" in m["时间戳来源"]["detail"] and \
            "毫秒" in m["时间戳来源"]["detail"], \
            f"比例过线时要把人指回上一站（转换脚本／毫秒戳），别让他一份份补：{m['时间戳来源']['detail']}"
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        #    ⚠ **这一格不许糊上「去查转换脚本」那句**（2026.08.12 同批补）：
        #    那句话的前提是"整批被吞掉"，而这里是"绝大多数块日期解析得好好的、
        #    只有少数几块没有"——病因完全不同，转换脚本没有吞任何东西。
        #    **同一条提示贴到不对的一格上，会把人支到一个根本没问题的地方去查。**
        #    变异：把那句提示的 `if ratio > _MTIME_WARN_RATIO` 条件去掉 → 这条红。
        assert "转换脚本" not in out_mix, \
            f"少数块落 mtime 不是转换脚本吞了整批，不许把那句提示糊过来：{out_mix}"
